...
- **Implémentation LIVRÉE 2026-08-02 — carte « plancher » sur la page Service (25-duty-tracker.js, _dutyWorkFloor)** : affiche « au moins X h » sur 7 dates civiles locales, calculé UNIQUEMENT sur les périodes de service enregistrées (présentation ci_utc → libération co_utc, regroupées par période de service : une période = suite de segments dont un seul porte un check-in ; jamais de repli approximatif). Le plafond n’est JAMAIS affiché comme un chiffre unique : la carte cite « 60 h ou 70 h selon les journées sans service accordées », dit que la réserve et l’attente comptent au règlement mais pas au carnet, compte à voix haute les vols non comptables, et précise qu’un plancher sous le plafond ne prouve pas la conformité. Détection positive seulement : plancher ≥ 60 h → avertissement conditionnel ; ≥ 70 h → dépasse les deux plafonds possibles. Demande de Martin, verbatim : « je veux que tu code dans le duty le 60 heures max dans 7 jours de duty time … donc check in to check out » — codé dans les bornes du présent registre, pas au-delà.
