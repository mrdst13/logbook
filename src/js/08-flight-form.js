...
// The flight number and the airport pair out of an iCal SUMMARY.
//
// The feed publishes both of these shapes:
//   "PD274 YYC-YOW"          plain
//   "PD325 (T) YOW-YLW"      with a duty marker between the two
//
// Every caller used to read the route POSITIONALLY, as the second word. On a
// marked leg that word is "(T)", which is not an airport pair, so the leg was
// refused for having no route — and Martin's PD325 and PD326 of 2026-09-11 were
// dropped on exactly that, invisible to the logbook, to the duty projection and
// to the registration backfill alike. The pair is found by SHAPE instead, so a
// marker anywhere in the summary is harmless. Markers are returned rather than
// interpreted: what "(T)" means is the operator's business, and this app does
// not invent meanings for codes it has not verified.
function icalSummaryLeg(summary) {
  const s = String(summary || '').trim();
  const pair = s.match(/\b([A-Z]{3})-([A-Z]{3})\b/);
  const num = s.match(/^\s*([A-Z]{1,3}\d{2,5})\b/);
  return {
    flightNum: num ? num[1] : ((s.split(/\s+/)[0]) || ''),
    depIATA: pair ? pair[1] : '',
    arrIATA: pair ? pair[2] : '',
    markers: (s.match(/\(([A-Za-z])\)/g) || []).map(x => x.slice(1, -1).toUpperCase())
  };
}
...
function rosterEventDropReason(ev) {
  ...
  const _leg = icalSummaryLeg(summary);
  if (!_leg.depIATA || !_leg.arrIATA) return 'no-route';
...
function navblueEventToFlight(ev, isFO, autoCountIFR) {
  ...
  // Parse SUMMARY: "PD274 YYC-YOW" or "PD325 (T) YOW-YLW"
  const _leg = icalSummaryLeg(summary);
  const flightNum = _leg.flightNum;
  const depIATA = _leg.depIATA, arrIATA = _leg.arrIATA;
...
function rosterRegistrationsFromEvents(events) {
  ...
  (events || []).forEach(ev => {
    ...
    const desc = String((ev && ev.DESCRIPTION) || '').trim();
    const _leg = icalSummaryLeg(summary);
    const flightNum = _leg.flightNum.toUpperCase();
    const depIATA = _leg.depIATA, arrIATA = _leg.arrIATA;
