...
// Country of a stored airport code, THREE-state. A 4-letter ICAO starting
// with K is US; any other 4-letter ICAO is not US; a code still 3 letters
// after iataToIcao (a station outside the 60-entry table, e.g. RSW, SAN,
// SEA, ATL) is UNKNOWN and must never be treated as Canadian. It used to
// be: an overnight in Fort Myers was paid as a Canadian layover, and the
// page then reported a phantom shortfall against a stub that had paid the
// US rate correctly. Same rule isCrossCountry already applies: say unknown
// rather than guess. (Audit 2026-07-27.)
function _payCountry(code) {
...
// A REAL LAYOVER vs A TURN. Martin, 2026-08-02, verbatim: "le us per diem est
// seulement paye pour un vrai layover la cetait un turn donc paye en cad". So
// the country of the station only matters when the crew actually laid over
// there; the ground time of a turn is paid at the Canadian rate wherever the
// airplane happens to be parked.
//
// The discriminator is read from his own roster, never guessed: a layover ends
// with a NEW duty period, and the leg out of it carries its own check-in
// (PD1326 out of the YLW layover publishes CI 1330Z). The return leg of a turn
// is the same duty period and carries none (PD326 of the Aug 4 YLW turn has
// no CI). No duration threshold is invented; the feed's own check-in is the
...
function pairingPerDiem(pairing) {
  ...
  for (let i = 0; i < pairing.length - 1; i++) {
    ...
    if (_payIsUnknownCountry(pairing[i].arr_icao)) {
      unknownStations++;
      // Its ground time leaves BOTH pools. Subtracting only from the US side
      // would have dropped it straight into the Canadian one by arithmetic.
      const us = _payReleaseMs(pairing[i]), ue = _payReportMs(pairing[i + 1]);
      if (!isNaN(us) && !isNaN(ue)) unknownMin += Math.max(0, (ue - us) / 60000);
      continue;
    }
  ...
  unknownMin = Math.min(unknownMin, Math.max(0, awayMin - usMin));
  return { awayHours: awayMin / 60, usHours: usMin / 60,
    cdnHours: (awayMin - usMin - unknownMin) / 60,
    unknownHours: unknownMin / 60, unknownStations: unknownStations };
}
...
function payRender() {
  ...
  const pill = s => s === 'ok'
    ...
    : s === 'bad'
      ? '<span class="pill-bad">' + (fr ? 'Problème' : 'Issue') + '</span>'
  ...
  } else if (pd.unknownStations > 0) {
    // A layover this app cannot place in a country is counted as neither
    // Canadian nor US, so the comparison would be against a figure it knows
  ...
  // The grid rate is a CALENDAR-YEAR figure and the stored seat year is
  // TODAY's step. Neither is known for a past period, so an auto-derived rate
  // is only comparable against a stub from the year it was derived for. It
  // used to be compared against every stub, so a December 2025 stub paying
  // the correct year-3 rate was reported as a pay error, with the page
  // telling him to change a setting that was already right. A rate he typed
  // himself is his own figure and stays comparable. (Audit 2026-07-27.)
  const _stubYear = perDmy ? perDmy.y : +String(ym).slice(0, 4);
  const _rateComparable = !!st.rateManual || _stubYear === new Date().getFullYear();
  if (stubRate != null && st.rate && !_rateComparable) {
    rateCheck.status = 'info';
    rateCheck.desc = fr
      ? 'Le talon montre ' + money(stubRate) + '/h. Le taux attendu vient de la grille de l’annee en cours, pas de ' + _stubYear + ', donc il n’est pas compare ici.'
      : 'The stub shows ' + money(stubRate) + '/h. The expected rate comes from this year’s grid, not ' + _stubYear + ', so it is not compared here.';
  } else if (stubRate != null && st.rate) {
  ...
  }
  {
    ...
    // The per-US-day breakdown (usPerDiemDays) — the view this helper was
    // written for and never wired to. One sub-row per confirmed US layover
    // day, so a wrong total is traceable to the day that caused it.
    // (Martin's go, 2026-08-02.)
    if (pd.usHours > 0 && typeof usPerDiemDays === 'function') {
      const _usDays = usPerDiemDays(allFls || [], st.base);
      _usDays.forEach(d => {
        usCheck.rows.push(tr('<span class="sub">' + esc(d.icao) + ' · ' + esc(d.date) + '</span>',
          tdN(hL(d.hours) + ' h') + tdM(DASH) + tdM(''), false));
      });
    }
  }
  ...
  };
  {
    ...
    const _hoursBad = baseCheck.status === 'bad' && paidHours != null && creditH != null && paidHours + 1.0 < creditH;
    baseCheck.rows = [
      tr(fr ? 'Heures créditées' : 'Credited hours',
        tdN(creditH != null ? hL(creditH) + ' h' : DASH) + tdN(paidHours != null ? hL(paidHours) + ' h' : DASH) +
        (_hoursBad ? tdBad(sgnH(Math.round((paidHours - creditH) * 100) / 100)) : tdP(pill(baseCheck.status))), _hoursBad),
      tr(fr ? 'Base et temps supp. (montants)' : 'Flight pay and overtime (amounts)',
        tdM(DASH) + tdN((baseAmt != null || otAmt != null) ? money(+(baseAmt || 0) + +(otAmt || 0)) : DASH) + tdP(pill(baseCheck.status)), false)
    ];
  }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { groupPairings, pairingPerDiem, computePerDiem, computeCredits, computeBasePay, isSummerLOA, payRateForYear, deriveUsFx, usPerDiemDays, _payMonthPairingLegs, computePerDiemInPeriod, _payLocalMidnightMs, _payNextDay, _payCountry };
}
